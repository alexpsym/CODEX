...
set "MASTER_ENV_PROTECTED_KEYS=APP_PROFILE,TRADING_JOURNAL_SOURCE,TRADING_JOURNAL_ENABLE_LOCAL_IMPORT,TRADING_JOURNAL_BROKER_REFRESH_ENABLED,TRADING_JOURNAL_BYBIT_DEMO_BALANCE_ANCHOR_ENABLED,TRADING_JOURNAL_LOCAL_DIR,TRADING_JOURNAL_GITHUB_SYNC_ENABLED,TRADING_JOURNAL_GITHUB_SYNC_REMOTE,TRADING_JOURNAL_GITHUB_SYNC_BRANCH,TRADING_JOURNAL_GITHUB_SYNC_INCLUDE_SOURCES,DROPBOX_SYNC_ENABLED,LOCAL_STATE_ONLY"
...
if not exist "%TRADING_JOURNAL_LOCAL_DIR%\" (
  mkdir "%TRADING_JOURNAL_LOCAL_DIR%" 2>nul
  if errorlevel 1 (
    echo [local-master] ERROR: failed to create TRADING_JOURNAL_LOCAL_DIR at %TRADING_JOURNAL_LOCAL_DIR%
    exit /b 1
  )
)
"%PYTHON_EXE%" "%ROOT%tools\ensure_local_journal_templates.py" "%TRADING_JOURNAL_LOCAL_DIR%"
if errorlevel 1 (
  echo [local-master] ERROR: failed to ensure local journal templates at %TRADING_JOURNAL_LOCAL_DIR%
  exit /b 1
)
