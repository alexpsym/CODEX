...
`InpShowBlackoutStatus` displays a compact chart status during the weekend block. `InpShowDiagnostics` adds the current mode, setup stage, opposing-close count and current gate reason. `InpShowBlackoutZones` adds one subtle historical rectangle per completed weekend blackout interval in Strategy Tester visual mode. Only objects named by this EA's `RBP_FX_BLACKOUT_` prefix are created.
...
Enable **Show blackout status** and **Show diagnostics** when investigating a missed setup. FT6 records compact Journal messages only when the gate/state changes, including blackout start/end, setup progression, reset and entry request; it does not print on every completed bar. Its supplied strategy API does not offer a clean historical chart-background shading equivalent, so this intentionally avoids creating many chart objects. Pine's Data Window gate code uses: 1 FX, 2 busy, 3 range, 4 impulse, 5 extension, 6 pullback, 7 correction/depth, 8 confirmation, 9 ATR/risk, 10 entry. Across platforms, the main skip categories are blackout, position busy, range/impulse not qualified, extension/pullback/retracement/depth failure, resumption failure, expired setup, or ATR risk not ready.
