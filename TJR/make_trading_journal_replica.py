...
def instrument_stats(trades: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for sym,items in buckets.items():
        ...
        def dist_pct(kind, row):
            e,sl,tp=safe_float(row.get("entry")),safe_float(row.get("stop_loss")),safe_float(row.get("take_profit"))
            if e in (None, 0): return None
            if kind=="sl" and sl is not None: return abs(e-sl)/e*100.0
            if kind=="tp" and tp is not None: return abs(tp-e)/e*100.0
            return None
        durs=metric_values(items,"trade_duration_seconds")
        pnl=[safe_float(x.get("net_profit")) for x in items if safe_float(x.get("net_profit")) is not None]
        out.append({"symbol":sym,"asset_class":clean_text(items[0].get("asset_class","")).lower(),"total_trades":len(items),"long_trades":sum(1 for x in items if _is_long_side(x)),"short_trades":sum(1 for x in items if _is_short_side(x)),"wins":len(wins),"losses":len(losses),"break_even":sum(1 for x in items if is_be(x)),"long_wins":sum(1 for x in items if _is_long_side(x) and is_win(x)),"long_losses":sum(1 for x in items if _is_long_side(x) and is_loss(x)),"long_break_even":sum(1 for x in items if _is_long_side(x) and is_be(x)),"short_wins":sum(1 for x in items if _is_short_side(x) and is_win(x)),"short_losses":sum(1 for x in items if _is_short_side(x) and is_loss(x)),"short_break_even":sum(1 for x in items if _is_short_side(x) and is_be(x)),"net_profit_total":sum(pnl) if pnl else None,"avg_net_profit":avg(pnl),"win_rate_pct":(len(wins)/(len(wins)+len(losses))*100.0) if (len(wins)+len(losses)) else None,"avg_trade_duration_seconds":avg(durs),"min_trade_duration_seconds":safe_min(durs),"max_trade_duration_seconds":safe_max(durs),"avg_sl_pct":avg([dist_pct("sl",x) for x in items if dist_pct("sl",x) is not None]),"avg_tp_pct":avg([dist_pct("tp",x) for x in items if dist_pct("tp",x) is not None]),"avg_sl_pct_wins":avg([dist_pct("sl",x) for x in wins if dist_pct("sl",x) is not None]),"avg_sl_pct_losses":avg([dist_pct("sl",x) for x in losses if dist_pct("sl",x) is not None]),"avg_tp_pct_wins":avg([dist_pct("tp",x) for x in wins if dist_pct("tp",x) is not None]),"avg_tp_pct_losses":avg([dist_pct("tp",x) for x in losses if dist_pct("tp",x) is not None]),"quote_currency":items[0].get("currency") or "USDT"})
