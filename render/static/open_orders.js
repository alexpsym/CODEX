(() => {
  ...
  const formatSourceErrors=(errors=[])=>Array.isArray(errors)?errors.map((entry)=>{if(!entry||typeof entry!=='object') return null; const broker=String(entry.broker||'Source').trim(); const account=String(entry.account||'').trim(); const category=String(entry.category||'').trim(); const message=String(entry.message||'').trim()||'Unknown source error'; return [broker,account,category].filter(Boolean).join(' ')+`: ${message}`;}).filter(Boolean):[];
  ...
  const render=(items,errors=[])=>{window.__openOrdersItems=Array.isArray(items)?items:[]; window.__openOrdersErrors=Array.isArray(errors)?errors:[]; if(!tbody) return; tbody.innerHTML=''; hasData=Boolean(items.length); if(errorsBox) errorsBox.style.display=errors.length?'block':'none'; if(errorsList){errorsList.innerHTML=''; errors.forEach((entry)=>{const li=document.createElement('li'); li.textContent=formatSourceErrors([entry])[0]||entry.message||'Unknown error'; errorsList.appendChild(li);});} if(!items.length){if(emptyState) emptyState.style.display='block'; return;} if(emptyState) emptyState.style.display='none';
