(() => {
  ...
  const refresh=async()=>{ if(refreshInFlight) return refreshInFlight; refreshInFlight=(async()=>{ try{setBadge('Loading...'); const payload=await fetchJson('/api/open-orders?force=1'); render(payload.items||[],payload.errors||[]); const stale=Boolean(payload.stale); const errCount=Array.isArray(payload.errors)?payload.errors.length:0; setBadge(stale?`Stale${errCount?` (${errCount} errors)`:''}`:`Updated${errCount?` (${errCount} errors)`:''}`);}catch(err){const reason=safeDiagnostic(err?.message||err,'request failed without diagnostic details').replace(/:+\s*$/,'').trim()||'request failed without diagnostic details'; setBadge(`Stale (refresh failed: ${reason})`);} finally{refreshInFlight=null;}})(); return refreshInFlight; };
