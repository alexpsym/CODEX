...
def _account_label(account: str) -> str:
    mode = str(account or "").strip().lower()
    if mode == "demo":
        return "OANDA DEMO"
    if mode == "live":
        return "OANDA LIVE"
    raise ValueError("account must be demo or live")
    ...
    *,
    ...
    return {
        ...
        "diagnostics": {
            ...
            "canonical_oanda_rows": canonical_count,
        },
    }
...
def repair(
    *,
    ...
    account_mode = str(account or "").strip().lower()
    account_label = _account_label(account_mode)
    ...
    if not canonical:
        raise RuntimeError(f"No canonical {account_label} rows parsed from {csv_path}")

    prepared, stale_ids = master_service._prepare_oanda_canonical_replacements(
        existing,
        canonical,
    )
    existing = master_service._sanitize_oanda_commission_fields(
        existing,
    ...
    repaired = master_service._sanitize_oanda_commission_fields(
        by_id.values(),
        raw_export_rows=prepared,
    )
    repaired_by_id = {
...
def main() -> None:
    ...
    parser.add_argument("--account", choices=["demo", "live"], required=True)
    ...
    parser.add_argument(
        "--update-workbook",
        action="store_true",
        help="Rewrite Trading Journal.xlsx in addition to persisted JSON state.",
    )
    ...
    result = repair(
        ...
        update_workbook=bool(args.update_workbook),
    )
