...
void Status(bool blocked){if(ShowBlackoutStatus&&blocked)Print((PChar)"Range Breakout-Pullback: FX WEEKEND BLACKOUT");else if(ShowDiagnostics)Print((PChar)(SetupModeSetting==0?(stage==0?"Range diagnostic: waiting for range/breakout":"Range diagnostic: active setup"):(stage==0?"Impulse diagnostic: waiting for qualifying impulse":"Impulse diagnostic: active setup")));}
