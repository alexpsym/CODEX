...
double ImpulseMinimumBodyATR=1.0,ImpulseMinimumRangeATR=1.25,ImpulseMinimumRetracementATR=0.50,ImpulseMaximumDepth=75.0; int ImpulseMaximumPullbackBars=30;
...
void Status(bool blocked){if(ShowBlackoutStatus&&blocked){EmitStatus((PChar)"RBP: FX weekend blackout");return;}if(!ShowDiagnostics)return;PChar reason=stage==0?(SetupModeSetting==0?(PChar)"RBP: range/breakout not qualified":(PChar)"RBP: impulse body/range not qualified"):stage==1?(PChar)"RBP: extension pending":stage==2?(pullbackStarted?(opposing<MinimumOpposingCloses?(PChar)"RBP: insufficient opposing closes":(PChar)"RBP: ATR retracement/depth pending"):(PChar)"RBP: pullback not started"):stage==3?(PChar)"RBP: resumption or ATR/risk pending":(PChar)"RBP: entry requested";EmitStatus(reason);}
...
void ProcessBar()
{
  ...
  if(stage==0&&SetupModeSetting==1&&!StrategyPositionOpen()){bool lng=TradeDirection!=2&&c>o&&c-o>=rmaATR*ImpulseMinimumBodyATR&&h-l>=rmaATR*ImpulseMinimumRangeATR;bool sht=TradeDirection!=1&&c<o&&o-c>=rmaATR*ImpulseMinimumBodyATR&&h-l>=rmaATR*ImpulseMinimumRangeATR;if(lng||sht){dir=lng?1:-1;stage=2;activeImpulseSetup=true;frozenR=dir==1?o:0;frozenS=dir==-1?o:0;setupATR=rmaATR;setupExtreme=dir==1?h:l;breakout=extension=nbar;pullbackStarted=false;opposing=0;hasMinorSwing=false;}}
