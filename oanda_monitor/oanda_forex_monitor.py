...
def evaluate_custom_alerts(
    ...
    for alert in enabled_alerts:
        ...
        if triggered and armed and can_trigger:
            ...
            log(msg)
            send_notification(
                "OANDA Custom Move Alert",
                msg,
                event={
                    "source": "OANDA",
                    "symbol": symbol,
                    "alert_type": "Custom move",
                    "condition": f"{resolved_dir} {threshold}{unit_label} in {window_s}s",
                    "current_value": current,
                },
            )
...
def send_notification_test() -> Dict[str, object]:
    title = "OANDA monitor notification test"
    message = "If you received this, OANDA alert notifications are working."
