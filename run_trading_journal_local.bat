...
echo [journal-local] TRADING_JOURNAL_LOCAL_DIR=%TRADING_JOURNAL_LOCAL_DIR%
if not exist "%TRADING_JOURNAL_LOCAL_DIR%\" (
  echo [journal-local] ERROR: TRADING_JOURNAL_LOCAL_DIR not found at %TRADING_JOURNAL_LOCAL_DIR%
  exit /b 1
)
if not exist "%TRADING_JOURNAL_LOCAL_DIR%\account_cashflows.xlsx" (
  echo [journal-local] ERROR: required workbook missing: %TRADING_JOURNAL_LOCAL_DIR%\account_cashflows.xlsx
  exit /b 1
)
set "MASTER_ENV_PROTECTED_KEYS=APP_PROFILE,TRADING_JOURNAL_SOURCE,TRADING_JOURNAL_ENABLE_LOCAL_IMPORT,TRADING_JOURNAL_BROKER_REFRESH_ENABLED,TRADING_JOURNAL_LOCAL_DIR,DROPBOX_SYNC_ENABLED,LOCAL_STATE_ONLY"
