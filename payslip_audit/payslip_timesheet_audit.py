...
def extract_timesheet_entries(
    ...
    seen_entries: Optional[Dict[date, Set[Tuple[str, Decimal]]]] = None,
) -> None:
    ...
    if seen_entries is None:
        seen_entries = {
            dt: {(entry.label.lower(), entry.hours) for entry in day_entries}
            for dt, day_entries in entries.items()
        }
    else:
        for dt, day_entries in entries.items():
            seen_entries.setdefault(
                dt, {(entry.label.lower(), entry.hours) for entry in day_entries}
            )
