...
const source = fs.readFileSync(process.argv[1], 'utf8');
const listeners = {};
function element(id) { return { id, style: {}, classList: { add: () => {}, remove: () => {} }, textContent: '', value: '', disabled: false, files: [], focusCount: 0, focus() { this.focusCount += 1; }, click: () => {}, after: () => {}, addEventListener: (type, fn) => { listeners[id + ':' + type] = fn; } }; }
const elements = {
