...
const fetch=async(url,options={})=>{
  ...
  if(url.includes('/custom-alerts')&&method==='POST'){const payload=JSON.parse(options.body);const alert={id:payload.id||(url.includes('/api/oanda-alerts/')?'oanda-saved':'bybit-saved'),...payload};if(alert.expires_at===null)delete alert.expires_at;savedPayloads.push(payload);savedRequests.push({url,payload,alert});return {ok:true,json:async()=>({ok:true,alert}),text:async()=>JSON.stringify({ok:true,alert})};}
...
(async()=>{
  ...
  editButtons()[0].dispatch('click'); expiry.value='lifetime'; save().dispatch('click'); await settle(); if(savedPayloads.at(-1).expires_at!==null) throw new Error('Lifetime edit did not explicitly clear current expiry'); if(Object.hasOwn(savedRequests.at(-1).alert,'expires_at')) throw new Error('Lifetime edit response retained expiry'); if(!allNodes(section).some((node)=>String(node.textContent||'').includes('No expiry'))) throw new Error('Lifetime edit was not displayed without expiry');
