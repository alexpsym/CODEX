...
def test_pepperstone_automatic_buffer_requires_no_user_input() -> None:
...
const calls=[];const reply=data=>({ok:true,status:200,statusText:'OK',headers:{get:()=> 'application/json'},text:async()=>JSON.stringify(data)});const fetch=async(url,opts={})=>{const body=opts.body?JSON.parse(opts.body):null;calls.push({url,body});if(url==='/api/calculator/bootstrap')return reply({app_profile:'local',trendline_plans_available:false,webhook:{available:true}});if(url.startsWith('/api/calculator/instrument'))return reply({asset:'fx',broker:'pepperstone',symbol:'USD_JPY'});if(url==='/api/calculator/quote')return reply({asset:'fx',broker:'pepperstone',venue:'Pepperstone',resolved_venue:'Pepperstone',symbol:'USD_JPY',tick_size:'0.001',entry_price:'150',stop_price:'149.957',target_price:'150.086',quantity:'1',estimated_total_loss_aud:'10',estimated_total_loss:'10',pepperstone_risk_buffer_preflight:{stop_points:43,preferred_maximum_buffer_points:50,minimum_automatic_buffer_points:10,conservative_max_buffer_points:14,planned_buffer_points:14,minimum_compatible_stop_points:30,compatible:true}});if(url.startsWith('/api/calculator/journal-summary')||url.startsWith('/api/instrument-specs'))return reply({status:'no_data'});return reply({status:'no_data'});};
...
(async()=>{await flush();await flush();el['asset-toggle'].buttons.find(b=>b.dataset.v==='fx').click();await flush();await flush();el['broker-toggle'].buttons.find(b=>b.dataset.v==='pepperstone').click();await flush();await flush();await el['calc-quote'].listeners.click();console.log(JSON.stringify({message:el['calc-pepperstone-risk-preflight'].textContent,visible:el['calc-pepperstone-set'].style.display!== 'none',enabled:!el['calc-pepperstone-set'].disabled,quotePayload:calls.find(c=>c.url==='/api/calculator/quote').body}));})().catch(e=>{console.error(e);process.exitCode=1;});
'''
    result = subprocess.run([node, "-e", harness, str(JS_PATH)], check=True, capture_output=True, text=True)
    data = json.loads(result.stdout.strip().splitlines()[-1])
    assert "calc-pepperstone-risk-buffer" not in JS_PATH.read_text(encoding="utf-8")
    assert "risk_slippage_buffer_points" not in JS_PATH.read_text(encoding="utf-8")
    assert data["visible"] is True and data["enabled"] is True
    assert "Automatic risk buffer ready: calculator cap 14 MT5 points" in data["message"]
    assert "between 10 and 14" in data["message"]
    assert "risk_slippage_buffer_points" not in data["quotePayload"]
