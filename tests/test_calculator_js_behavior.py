...
def test_pepperstone_risk_buffer_preflight_controls_set_download() -> None:
...
class B { constructor(v){this.dataset={v};this.listeners={};this.disabled=false;this.attrs={};this.classList={toggle(){},add(){},remove(){}};} addEventListener(e,f){this.listeners[e]=f;} click(){return this.listeners.click&&this.listeners.click();} setAttribute(k,v){this.attrs[k]=String(v);} getAttribute(k){return this.attrs[k];} removeAttribute(k){delete this.attrs[k];} }
class E extends B { constructor(id){super('');this.id=id;this.value='';this.textContent='';this.style={};this._html='';this.buttons=[];this.checked=false;} set innerHTML(v){this._html=String(v||'');this.buttons=[...this._html.matchAll(/data-v="([^"]*)"/g)].map(m=>new B(m[1]));} get innerHTML(){return this._html;} querySelectorAll(s){return s==='button'?this.buttons:[];} }
const ids=['calc-error','calc-error-debug','calc-success','calc-results','calc-request-summary','calc-canonical-symbol','calc-journal-summary','calc-instrument-specs','risk-toggle-wrap','pepperstone-risk-buffer-wrap','calc-pepperstone-risk-buffer','calc-pepperstone-risk-preflight','calc-webhook-panel','calc-webhook-url','calc-webhook-json','calc-webhook-copy','calc-webhook-copy-url','risk-toggle','calc-risk-label','limit-wrap','account-toggle','asset-toggle','side-toggle','order-toggle','webhook-toggle','test-toggle','timeframe-toggle','setup-toggle','pattern-toggle','ema-toggle','vwap-toggle','aths-atls-toggle','round-number-toggle','calc-symbol','calc-limit','calc-sl-ticks','calc-rr','calc-risk','calc-quote','calc-submit','calc-quote-status','calc-webhook-status','calc-pepperstone-set','broker-toggle-wrap','broker-toggle','trendline-plans-panel','trendline-anchor-1-time','trendline-anchor-1-price','trendline-anchor-1-utc','trendline-anchor-2-time','trendline-anchor-2-price','trendline-anchor-2-utc','trendline-trigger-mode','trendline-cross-direction','trendline-price-basis','trendline-tolerance-ticks','trendline-right-extension','trendline-expiry','trendline-save','trendline-reset','trendline-refresh','trendline-plan-status','trendline-plan-list','trendline-monitor-start','trendline-monitor-stop','trendline-monitor-status'];
const el=Object.fromEntries(ids.map(i=>[i,new E(i)]));for(const [id,vals] of Object.entries({'risk-toggle':['fixed_aud','percent'],'asset-toggle':['crypto','fx'],'broker-toggle':['oanda','pepperstone'],'account-toggle':['live','demo'],'side-toggle':['buy','sell'],'order-toggle':['market','limit'],'webhook-toggle':['no','yes'],'test-toggle':['no','yes']}))el[id].buttons=vals.map(v=>new B(v));
Object.assign(el['calc-symbol'],{value:'USD_JPY'});el['calc-sl-ticks'].value='43';el['calc-rr'].value='2';el['calc-risk'].value='10';
const calls=[];const reply=data=>({ok:true,status:200,statusText:'OK',headers:{get:()=> 'application/json'},text:async()=>JSON.stringify(data)});const fetch=async(url,opts={})=>{const body=opts.body?JSON.parse(opts.body):null;calls.push({url,body});if(url==='/api/calculator/bootstrap')return reply({app_profile:'local',trendline_plans_available:false,pepperstone_risk_slippage_buffer_points:50,webhook:{available:true}});if(url.startsWith('/api/calculator/instrument'))return reply({asset:'fx',broker:'pepperstone',symbol:'USD_JPY'});if(url==='/api/calculator/quote'){const b=Number(body.risk_slippage_buffer_points),ok=b<=14;return reply({asset:'fx',broker:'pepperstone',venue:'Pepperstone',resolved_venue:'Pepperstone',symbol:'USD_JPY',tick_size:'0.001',entry_price:'150',stop_price:'149.957',target_price:'150.086',quantity:'1',estimated_total_loss_aud:'10',estimated_total_loss:'10',pepperstone_risk_buffer_preflight:{stop_points:43,selected_buffer_points:b,effective_stop_points:43+b,risk_min_aud:'9',risk_max_aud:'12',conservative_buffered_risk_at_min_aud: b===50?'19.47':'11.93',conservative_buffered_risk_at_target_aud:'13.26',conservative_max_buffer_points:14,compatible:ok}});}if(url.startsWith('/api/calculator/journal-summary')||url.startsWith('/api/instrument-specs'))return reply({status:'no_data'});return reply({status:'no_data'});};
const context={document:{getElementById:id=>el[id]},fetch,navigator:{clipboard:{writeText:async()=>{}}},setTimeout:fn=>{fn();return 1;},clearTimeout(){},setInterval:()=>1,clearInterval(){},console,Date,URL,URLSearchParams,AbortController};vm.runInNewContext(source,context);const flush=()=>new Promise(r=>setImmediate(r));
(async()=>{await flush();await flush();el['asset-toggle'].buttons.find(b=>b.dataset.v==='fx').click();await flush();await flush();el['broker-toggle'].buttons.find(b=>b.dataset.v==='pepperstone').click();await flush();await flush();const defaultBuffer=el['calc-pepperstone-risk-buffer'].value,visible=el['pepperstone-risk-buffer-wrap'].style.display!== 'none';await el['calc-quote'].listeners.click();const blocked=el['calc-pepperstone-set'].disabled,blockedVisible=el['calc-pepperstone-set'].style.display!== 'none',message=el['calc-pepperstone-risk-preflight'].textContent;el['calc-pepperstone-risk-buffer'].value='14';el['calc-pepperstone-risk-buffer'].listeners.input();const invalidated=el['calc-pepperstone-set'].style.display==='none';await el['calc-quote'].listeners.click();console.log(JSON.stringify({defaultBuffer,visible,blocked,blockedVisible,message,invalidated,enabled:!el['calc-pepperstone-set'].disabled,quoteBuffers:calls.filter(c=>c.url==='/api/calculator/quote').map(c=>c.body.risk_slippage_buffer_points),setPayloadIncluded:!calls.some(c=>c.url==='/api/calculator/pepperstone-set')}));})().catch(e=>{console.error(e);process.exitCode=1;});
'''
    result = subprocess.run([node, "-e", harness, str(JS_PATH)], check=True, capture_output=True, text=True)
    data = json.loads(result.stdout.strip().splitlines()[-1])
    assert data["defaultBuffer"] == "50"
    assert data["visible"] is True
    assert data["blocked"] is True and data["blockedVisible"] is True
    assert "stop 43 points + selected buffer 50 = 93 points" in data["message"]
    assert "AUD 19.47" in data["message"] and "AUD 12 cap" in data["message"]
    assert "Maximum compatible buffer: 14 points" in data["message"]
    assert data["invalidated"] is True
    assert data["enabled"] is True
    assert data["quoteBuffers"] == ["50", "14"]
    assert data["setPayloadIncluded"] is True
